<script lang="ts">
	...
	let hoyoSlopDailyReset = $state(DateTime.utc().set({ hour: 9, minute: 0, second: 0, millisecond: 0 }));
	let gkmsDailyReset = $state(DateTime.utc().set({ hour: 20, minute: 0, second: 0, millisecond: 0 }));
	let arknightsDailyReset = $state(DateTime.utc().set({ hour: 11, minute: 0, second: 0, millisecond: 0 }));
	let fgoJPDailyReset = $state(DateTime.utc().set({ hour: 19, minute: 0, second: 0, millisecond: 0 }));
	let fgoENDailyReset = $state(DateTime.utc().set({ hour: 4, minute: 0, second: 0, millisecond: 0 }));
	...
	onMount(() => {
		...
		hoyoSlopDailyReset = hoyoSlopDailyReset.diff(time).valueOf() < 0 ? hoyoSlopDailyReset.plus({ days: 1 }) : hoyoSlopDailyReset;
		gkmsDailyReset = gkmsDailyReset.diff(time).valueOf() < 0 ? gkmsDailyReset.plus({ days: 1 }) : gkmsDailyReset;
		arknightsDailyReset = arknightsDailyReset.diff(time).valueOf() < 0 ? arknightsDailyReset.plus({ days: 1 }) : arknightsDailyReset;
		fgoJPDailyReset = fgoJPDailyReset.diff(time).valueOf() < 0 ? fgoJPDailyReset.plus({ days: 1 }) : fgoJPDailyReset;
		fgoENDailyReset = fgoENDailyReset.diff(time).valueOf() < 0 ? fgoENDailyReset.plus({ days: 1 }) : fgoENDailyReset;
