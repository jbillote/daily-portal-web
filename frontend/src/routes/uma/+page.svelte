<script lang="ts">
  ...
  function updateTotal() {
    ...
    for (let day = 0; day < startValues.length; day++) {
      newTotal += Math.max(0, endValues[day] - startValues[day]);
    }
  ...
  <div>
    ...
    >
      <calendar-date
        class="cally"
        firstDayOfWeek={0}
        on:change={(e: Event) => (date = startOfWeek((e.target as HTMLInputElement).value))}
      >
        <div slot="previous">
    ...
    <div>
      <table class="m-auto border-separate border-spacing-2 border-none">
        <thead>
          <tr>
            <th>Date</th>
            <th>Starting Fans</th>
        ...
        <tbody>
          {#each { length: 7 }, offset}
            <tr>
              <td>{date.plus({ days: offset }).toLocaleString()}</td>
              <td>
                <input
                  type="text"
                  placeholder="Start"
                  ...
                  bind:value={
                    () => startValues[offset].toString(),
                    (v: string) => (startValues[offset] = parseInt(v))
                  }
                />
              </td>
              <td>
                <input
                  ...
                  on:change={updateTotal}
                  bind:value={
                    () => endValues[offset].toString(),
                    (v: string) => (endValues[offset] = parseInt(v))
                  }
                />
              </td>
            </tr>
